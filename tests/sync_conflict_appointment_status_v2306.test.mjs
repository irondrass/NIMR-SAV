...
console.log("Démarrage tests statut RDV canonique sync v23.2.1...");
...
// 13. Version/cache v23.2.1 validés
{
  ...
  assert.match(stateSource, /APP_VERSION\s*=\s*"v23\.2\.1"/, "state.js n'a pas la bonne version");
  assert.match(swSource, /nimr-sav-v23\.2\.1-reception-workflow-qa-status-stabilization/, "sw.js n'a pas le bon cache");
  assert.match(versionSource, /NIMR_BUILD\s*=\s*"v23\.2\.1"/, "version.js n'a pas la bonne version");
  assert.match(appSource, /sw\.js\?v=23\.2\.1/, "app.js n'appelle pas le bon sw.js");
  console.log("-> Test 13 (Version/cache v23.2.1) OK");
}

console.log("Tous les tests statut RDV canonique sync v23.2.1 passés avec succès !");
