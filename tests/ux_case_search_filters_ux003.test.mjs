...
check("A Release is consistently v23.3.10 across all runtime cache surfaces", () => {
  assert.match(versionSource, /window\.APP_VERSION = "v23\.3\.10";/u);
  assert.match(versionSource, /window\.NIMR_BUILD = "v23\.3\.10";/u);
  assert.match(versionSource, /window\.NIMR_CACHE_NAME = "nimr-sav-v23\.3\.10";/u);
  assert.match(stateSource, /const APP_VERSION = "v23\.3\.10";/u);
  assert.match(appSource, /pdf\.worker\.min\.js\?v=23\.3\.10/u);
  assert.match(swSource, /const CACHE_NAME = "nimr-sav-v23\.3\.10";/u);
  assert.match(indexSource, /styles\.css\?v=23\.3\.10/u);
  assert.match(indexSource, /app\.js\?v=23\.3\.10/u);
  assert.match(offlineSource, /styles\.css\?v=23\.3\.10/u);
});
