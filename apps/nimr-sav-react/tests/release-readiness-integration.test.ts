...
describe('SAV Release Readiness & Invariants Integration (v24.0.0-alpha.16)', () => {
  // 1. Version matches
  it('APP_VERSION is exactly v24.0.0-alpha.16', () => {
    expect(APP_VERSION).toBe('v24.0.0-alpha.16');
  });

  it('package.json version matches 24.0.0-alpha.16', () => {
    const pkgPath = resolve(__dirname, '../package.json');
    const pkg = JSON.parse(readFileSync(pkgPath, 'utf-8'));
    expect(pkg.version).toBe('24.0.0-alpha.16');
  });

  it('package-lock.json version matches 24.0.0-alpha.16', () => {
    const lockPath = resolve(__dirname, '../package-lock.json');
    const lock = JSON.parse(readFileSync(lockPath, 'utf-8'));
    expect(lock.version).toBe('24.0.0-alpha.16');
  });
  ...
  it('reports success for valid, clean cases', () => {
    ...
    const report = validateReleaseReadiness([mockCase], [mockLog], { appVersion: 'v24.0.0-alpha.16' });
  ...
  it('detects blockers for unofficial roles, statuses, or corrupted workflow state', () => {
    ...
    const report = validateReleaseReadiness([mockCaseBadStatus, mockCaseNoQc], [mockLogBadRole], { appVersion: 'v24.0.0-alpha.16' });
