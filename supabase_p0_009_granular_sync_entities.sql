...
-- One-time server-side bootstrap from the latest legacy disaster-recovery row.
-- This does not transfer a global JSON document to a browser; subsequent
-- clients read these rows through the compound-cursor pages below.
...
select
  ...
  payload,
  case
    when coalesce(payload ->> 'localRevision', '') ~ '^[0-9]+$'
      then (payload ->> 'localRevision')::bigint
    else 0
  end,
  'legacy-bootstrap:' || md5(workshop_id::text || ':case:' || entity_id),
  null,
...
select
  ...
  payload,
  case
    when coalesce(payload ->> 'version', payload ->> 'localRevision', '') ~ '^[0-9]+$'
      then coalesce(payload ->> 'version', payload ->> 'localRevision')::bigint
    else 0
  end,
  'legacy-bootstrap:' || md5(workshop_id::text || ':booking:' || entity_id),
  null,
  coalesce(updated_at, clock_timestamp()),
  coalesce(updated_at, clock_timestamp())
...
with latest_backup as (
  ...
  select
    ...
    item.payload,
    coalesce(
      nullif(item.payload ->> 'id', ''),
      'legacy-audit:' || md5(item.payload::text)
    ) as local_id,
    backup.updated_at
  from latest_backup backup
...
insert into public.audit_logs (
  workshop_id, local_id, repair_order_id, action, entity_type,
...
insert into public.app_settings (
  workshop_id, setting_key, value, description, updated_at
)
select
  workshop_id,
  'workshop_settings',
...
create policy nimr_sync_entities_update
...
begin
  ...
  -- A same-version tombstone wins over a different upsert. P0-010 owns the
  -- broader simultaneous-edit policy; this rule prevents ordinary stale retry
  -- from resurrecting a confirmed delete.
  if found
