...
await check("J service worker is source-refreshed without changing the v23.3.30 cache contract", () => {
  assert.match(sw, /UX-010 source refresh/u);
  assert.match(sw, /const CACHE_NAME = "nimr-sav-v23\.3\.30"/u);
  assert.match(read("js/version.js"), /^window\.APP_VERSION = "v23\.3\.30";$/mu);
});
