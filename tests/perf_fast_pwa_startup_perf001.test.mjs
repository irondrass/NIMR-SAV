...
function fixture(cached = []) {
  ...
  const ctx = vm.createContext({
    ...
    caches: {
      ...
      async keys() { return ["nimr-sav-v23.3.33"]; },
...
await check("A cache and app version remain v23.3.33", () => {
  assert.match(sw, /PERF-001 source refresh/u);
  assert.match(sw, /const CACHE_NAME = "nimr-sav-v23\.3\.33"/u);
  assert.match(read("js/version.js"), /^window\.APP_VERSION = "v23\.3\.33";$/mu);
});
