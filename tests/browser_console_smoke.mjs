...
const targetUrl = process.env.NIMR_BROWSER_TEST_URL || `http://127.0.0.1:${defaultPort}/?browser-smoke=22.21`;
  ...
  try {
    ...
    const pwaCheck = await send("Runtime.evaluate", {
      ...
      expression: `Promise.all([
        ...
        caches?.keys?.().then((keys) => keys.some((key) => key.includes('v22.21'))).catch(() => false),
      ]).then(([hasServiceWorker, hasV2221Cache]) => ({ hasServiceWorker, hasV2221Cache }))`,
    }, sessionId);
