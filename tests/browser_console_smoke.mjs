...
const targetUrl = process.env.NIMR_BROWSER_TEST_URL || `http://127.0.0.1:${defaultPort}/?browser-smoke=22.17`;
  ...
  try {
    ...
    const pwaCheck = await send("Runtime.evaluate", {
      ...
      expression: `Promise.all([
        ...
        caches?.keys?.().then((keys) => keys.some((key) => key.includes('v22.17'))).catch(() => false),
      ]).then(([hasServiceWorker, hasV2217Cache]) => ({ hasServiceWorker, hasV2217Cache }))`,
    }, sessionId);
