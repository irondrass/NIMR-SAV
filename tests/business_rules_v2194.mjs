...
assert.equal(quality,undefined,'aucun contrôle séparé ne doit être ajouté');
const finish=applied.find(l=>l.phase==='finish');
assert.equal(finish,undefined,'aucune finition/lavage synthétique ne doit être ajoutée');
assert.equal(JSON.stringify(applied.map((line)=>line.phase)),JSON.stringify(['prep','paint']));
