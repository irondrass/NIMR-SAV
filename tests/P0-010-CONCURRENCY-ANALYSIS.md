...
Scenarios A–W and X continue to pass. This includes five clock offsets, same-millisecond unique operation IDs, offline X1→X3 coalescing, delete/update and delete/recreate ABA, response loss, both realtime/ack orders, reordered realtime across restart, missed-event polling, restart durability, workshop isolation, real projection identity, old-client rejection, response-loss plus U2, atomic settlement failure, conflict restart, realtime with local pending, both case-delete/booking-upsert serial orders, settings CAS, existing-row unknown-base rejection, and accepted-receipt/current-canonical replay. Audit append idempotency and projection-failure retry also pass.
...
Accepted receipts still prove that the original operation succeeded and preserve its `accepted_version`. They no longer claim that the entity remains at that historical version. Restart followed by delayed realtime V101 likewise leaves V102 and the newer local intent intact.

### Historical-conflict/current-canonical corrective pass

The pre-fix Y1 characterization reproduced the merge blocker. A stale U1 conflicted against V101 and the server durably stored that evidence. A projection/network failure left U1 retryable; another station advanced both `sync_entities` and the real-identity `repair_orders` projection to V102. Retrying U1 returned the stored V101 row as `canonical`/`serverVersion`, allowing projection reconciliation to regress to V101.

Conflict responses now keep immutable detection evidence and current authority in separate fields. `conflictServerVersion`, `conflictCanonical`, `baseVersion`, `localPayload`, `serverPayload`, `detectedAt`, and the conflict ID describe the historical event. `serverVersion` and `canonical` are read from the current `sync_entities` or `app_settings` row. If the current row is absent, current canonical/version are null without discarding the historical conflict evidence. A rolling client against the old overloaded response fails closed: it classifies the legacy `canonical` as conflict evidence and does not project it as current.

Y1 now returns historical V101 and current V102 together, and only current V102 drives `repair_orders`; the deterministic canonical application ID differs from `repair_orders.local_id`. Y2 proves ACCEPT SERVER refreshes and persists V102 before removing the conflicted operation. Y3 proves KEEP LOCAL preserves the losing payload, generates a new operation ID with base V102, and is accepted as V103 when no write intervenes. Settings repeat the same V201/V202 split for both decisions. A production-path injected error after the durable `conflicted` transition leaves that terminal status intact and automatic flush excludes it, while pre-transition projection/network failures remain failed and retryable.

### Conflict-resolution authorization correction

`nimr_resolve_sync_entity_conflict` now locks and loads the target conflict by both workshop ID and conflict ID before authorizing resolution. The RPC derives authorization only from the stored `target_conflict.entity_type`; callers cannot supply or spoof an entity type. Case and booking conflicts allow `admin_technique`, `directeur`, `chef_atelier`, `reception`, and `technicien`. Workshop-settings conflicts allow the same roles except `technicien`. `controle_qualite` and `lecture_seule` remain denied, and unknown entity types fail closed.

A workshop/conflict mismatch returns no row without disclosing cross-workshop conflict data. Already-resolved conflicts return their locked historical row unchanged, so resolution retry neither recreates nor deletes evidence. SECURITY DEFINER, the fixed `pg_catalog, public` search path, RLS, authenticated-only execution, and the absence of direct browser UPDATE rights remain unchanged.

The deterministic SQL contract test proves the role matrix, server-derived entity type, workshop-scoped lock, fail-closed unknown type, and retry-safe resolved path. No live PostgreSQL/Supabase test environment is configured, so this corrective pass does not claim live database authorization validation.
