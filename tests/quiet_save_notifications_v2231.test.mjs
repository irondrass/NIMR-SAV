...
assert.match(stateJs, /APP_VERSION\s*=\s*"v23\.1A\.1"/, "state.js n'a pas la bonne version");
assert.match(swJs, /nimr-sav-v23\.1a\.1-pin-overlay-click-fix/, "sw.js n'a pas le bon cache");
assert.match(versionJs, /NIMR_BUILD\s*=\s*"v23\.1A\.1"/, "version.js n'a pas la bonne version");
assert.match(appJs, /sw\.js\?v=23\.1A\.1/, "app.js n'appelle pas le bon sw.js");
