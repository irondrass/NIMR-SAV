...
// PWA delivery contract: every runtime reference must force v23.3.3.
...
assert.match(versionSource, /window\.APP_VERSION = "v23\.3\.3"/u);
assert.match(versionSource, /window\.NIMR_CACHE_NAME = "nimr-sav-v23\.3\.3"/u);
assert.match(stateSource, /const APP_VERSION = "v23\.3\.3"/u);
assert.match(serviceWorkerSource, /const CACHE_NAME = "nimr-sav-v23\.3\.3"/u);
for (const match of indexSource.matchAll(/\?v=([0-9.]+)/gu)) assert.equal(match[1], "23.3.3");
assert.match(appSource, /serviceWorker\.register\("sw\.js\?v=23\.3\.3"/u);
assert.match(offlineSource, /styles\.css\?v=23\.3\.3/u);
