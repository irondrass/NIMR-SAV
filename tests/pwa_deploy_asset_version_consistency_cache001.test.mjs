...
test("B. NEW CACHE VERSION: bumping cache name purges old cache upon activation", async () => {
  ...
  const oldCache = await harness.context.caches.open("nimr-sav-v23.3.20");
  await oldCache.put(`${origin}/NIMR-SAV/js/storage.js?v=23.3.20`, new MockResponse("old-v20"));

  await harness.context.caches.open("nimr-sav-v23.3.21");
  await harness.triggerActivate();

  const keys = await harness.context.caches.keys();
  assert.ok(!keys.includes("nimr-sav-v23.3.20"), "Old cache nimr-sav-v23.3.20 must be deleted on activate");
  assert.ok(keys.includes("nimr-sav-v23.3.21"), "Current cache nimr-sav-v23.3.21 must be preserved");
});

test("C. HTML/JS VERSION MISMATCH: if index.html requests ?v=23.3.22 while cache only has ?v=23.3.21, it falls back to networkFirst", async () => {
  const harness = createSwHarness({
    [`${origin}/NIMR-SAV/js/unrelated.js?v=23.3.22`]: new MockResponse("network-v22"),
  });
  const cache = await harness.context.caches.open("nimr-sav-v23.3.21");
  await cache.put(`${origin}/NIMR-SAV/js/unrelated.js?v=23.3.21`, new MockResponse("cached-v21"));

  const res = await harness.dispatchFetch(`${origin}/NIMR-SAV/js/unrelated.js?v=23.3.22`);
  assert.equal(await res.text(), "network-v22");
});
...
test("G. OFFLINE CONTROL: when offline, cached assets resolve immediately", async () => {
  const storageUrl = `${origin}/NIMR-SAV/js/storage.js?v=23.3.21`;
  const harness = createSwHarness({});
  const cache = await harness.context.caches.open("nimr-sav-v23.3.21");
...
test("AA. VERSION BUMP COMPLETE: all 7 production files agree on v23.3.21 and contain zero runtime v23.3.20 literals", () => {
  ...
  assert.match(versionSrc, /window\.APP_VERSION = "v23\.3\.21";/);
  assert.match(versionSrc, /window\.NIMR_BUILD = "v23\.3\.21";/);
  assert.match(versionSrc, /window\.NIMR_CACHE_NAME = "nimr-sav-v23\.3\.21";/);
  assert.match(stateSrc, /const APP_VERSION = "v23\.3\.21";/);
  assert.match(swSrc, /const CACHE_NAME = "nimr-sav-v23\.3\.21";/);
  assert.match(appSrc, /vendor\/pdf\.worker\.min\.js\?v=23\.3\.21/);
  assert.match(appSrc, /sw\.js\?v=23\.3\.21/);
  assert.match(estimateSrc, /vendor\/pdf\.worker\.min\.js\?v=23\.3\.21/);
  assert.match(offlineSrc, /styles\.css\?v=23\.3\.21/);

  // Assert no runtime code in the 7 production files contains v23.3.20 (ignoring historical comments in sw.js)
  const codeFiles = [
  ...
  for (const { file, src } of codeFiles) {
    assert.doesNotMatch(src, /23\.3\.20/, `${file} must not contain any 23.3.20 literal`);
  }
});
...
test("AB. ACTIVE BUCKET ISOLATION: Worker B active with Cache B returns B even when Cache C exists with C", async () => {
  const sharedCaches = new Map();
  const network = new Map();
  const assetUrl = `${origin}/NIMR-SAV/js/storage.js?v=23.3.21`;

  const workerB = createSwHarness({}, { cacheStorage: sharedCaches, network });
  const cacheB = await workerB.context.caches.open("nimr-sav-v23.3.21");
  await cacheB.put(assetUrl, new MockResponse("// Storage Release B"));

  // Open and populate Cache C with C content
  const cacheC = await workerB.context.caches.open("nimr-sav-v23.3.22");
...
test("AC. WAITING CACHE INVISIBLE: matching URL in Cache C never satisfies Worker B lookup", async () => {
  const sharedCaches = new Map();
  const network = new Map();
  const missingAssetUrl = `${origin}/NIMR-SAV/js/state.js?v=23.3.21`;

  const workerB = createSwHarness({}, { cacheStorage: sharedCaches, network });
  // Cache B does NOT contain missingAssetUrl
  await workerB.context.caches.open("nimr-sav-v23.3.21");
...
test("AD. NO RELEASE BACKGROUND REVALIDATION: cache hit for versioned JS/CSS makes zero network calls and zero cache puts", async () => {
  const sharedCaches = new Map();
  const network = new Map();
  const jsUrl = `${origin}/NIMR-SAV/js/storage.js?v=23.3.21`;
  const cssUrl = `${origin}/NIMR-SAV/styles.css?v=23.3.21`;
  ...
  const cache = await worker.context.caches.open("nimr-sav-v23.3.21");
...
test("AG. CHECK_UPDATE NO-OP: CHECK_UPDATE message does not call precache, mutate Cache B, or emit false CACHE_REFRESHED", async () => {
  ...
  const worker = createSwHarness({}, { cacheStorage: sharedCaches, network });
  const cache = await worker.context.caches.open("nimr-sav-v23.3.21");
  await cache.put(`${origin}/NIMR-SAV/js/storage.js?v=23.3.21`, new MockResponse("// initial B bytes"));

  const snapshotBefore = Array.from(sharedCaches.get("nimr-sav-v23.3.21")?.entries() || [])
    .map(([k, v]) => [k, v.body]);
  ...
  // Cache B unchanged
  const stored = await cache.match(`${origin}/NIMR-SAV/js/storage.js?v=23.3.21`);
...
test("AI. CONTROLLED WAITING: new worker installs into its own bucket, stays waiting, old worker continues serving", async () => {
  ...
  // Worker C installs
  const swSourceC = swSource.replace(/23\.3\.21/g, "23.3.22");
...
test("AL. POST-ACTIVATION: Worker C active serves complete Release C with no Cache B dependency", async () => {
  ...
  const swSourceC = swSource.replace(/23\.3\.21/g, "23.3.22");
  const workerC = createSwHarness({}, { cacheStorage: sharedCaches, network, swSource: swSourceC });
  const cacheC = await workerC.context.caches.open("nimr-sav-v23.3.22");
  await cacheC.put(`${origin}/NIMR-SAV/`, new MockResponse("<!DOCTYPE html><html>App C</html>"));
  await cacheC.put(`${origin}/NIMR-SAV/js/storage.js?v=23.3.22`, new MockResponse("// Storage C"));

  await workerC.triggerActivate();

  const nav = await workerC.dispatchFetch(`${origin}/NIMR-SAV/`, "navigate");
  const js = await workerC.dispatchFetch(`${origin}/NIMR-SAV/js/storage.js?v=23.3.22`);
...
test("AM. OLD CACHE PRUNE: activate deletes older nimr-sav- buckets and preserves current", async () => {
  ...
  const worker = createSwHarness({}, { cacheStorage: sharedCaches, network });
  await worker.context.caches.open("nimr-sav-v23.3.19");
  ...
  assert.ok(!remainingKeys.includes("nimr-sav-v23.3.19"));
...
test("AN. PDF WORKER CONTRACT: app.js, estimate-import.js, and sw.js precache PDF worker URLs agree exactly", () => {
  ...
  assert.equal(appMatch[1], "vendor/pdf.worker.min.js?v=23.3.21");
  assert.equal(estimateMatch[1], "vendor/pdf.worker.min.js?v=23.3.21");
  assert.ok(swSrc.includes('"./vendor/pdf.worker.min.js?v=23.3.21"'));
});
...
test("AO. OFFLINE: complete release usable entirely from active cache when offline", async () => {
  ...
  await cache.put(`${origin}/NIMR-SAV/js/storage.js?v=23.3.21`, new MockResponse("// Storage"));
  await cache.put(`${origin}/NIMR-SAV/styles.css?v=23.3.21`, new MockResponse("/* Styles */"));

  // Completely empty network
  network.clear();

  const navRes = await worker.dispatchFetch(`${origin}/NIMR-SAV/`, "navigate");
  const jsRes = await worker.dispatchFetch(`${origin}/NIMR-SAV/js/storage.js?v=23.3.21`);
  const cssRes = await worker.dispatchFetch(`${origin}/NIMR-SAV/styles.css?v=23.3.21`);
...
// You MUST bump the release identity (e.g. v23.3.21 -> v23.3.22) and add a NEW entry.
...
function validateReleaseFingerprintContract({
  ...
  if (actualFingerprint !== sealedFingerprint) {
    throw new Error(
      ...
      `1. Bump release identity (e.g. ${appVersion} -> next version, e.g. v23.3.22) across the authorized production files\n` +
...
const EXPECTED_RELEASE_VERSION = "v23.3.21";
...
test("AT. SEALED RELEASE MUTATION: modifying runtime source under sealed v23.3.21 fails validation", () => {
  const originalContent = fs.readFileSync(path.join(rootDir, "js/storage.js"));
  const mutatedContent = Buffer.concat([originalContent, Buffer.from(" ")]);
...
test("AV. NEW RELEASE ENTRY: bumping release to v23.3.22 with new sealed entry passes while v23.3.21 stays sealed", () => {
  const simulatedRegistry = {
    ...SEALED_RELEASE_FINGERPRINTS,
    "v23.3.22": "1111111111111111111111111111111111111111111111111111111111111111",
  };
...
test("AS. isReleaseAsset MEMBERSHIP: only declared ASSETS with ?v=23.3.21 are classified as release assets", async () => {
  ...
  const arbitraryUrls = [
    `${origin}/NIMR-SAV/api/data?v=23.3.21`,
    `${origin}/NIMR-SAV/some-other-page?v=23.3.21&extra=true`,
    `${origin}/other-app/js/storage.js?v=23.3.21`,
  ];
  for (const url of arbitraryUrls) {
    ...
    // Therefore even if an arbitrary URL with ?v=23.3.21 reaches isReleaseAsset and it returns true,
