...
const targetUrl = process.env.NIMR_BROWSER_TEST_URL || `http://127.0.0.1:${defaultPort}/?browser-smoke=23.0.4`;
  ...
  try {
    ...
    const pwaCheck = await send("Runtime.evaluate", {
      ...
      expression: `Promise.all([
        ...
        caches?.keys?.().then((keys) => keys.some((key) => key.includes('v23.0.4'))).catch(() => false),
