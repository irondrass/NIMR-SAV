...
function configurePdfWorker() {
  if (window.pdfjsLib?.GlobalWorkerOptions) {
    window.pdfjsLib.GlobalWorkerOptions.workerSrc = "vendor/pdf.worker.min.js?v=23.3.55";
  }
}
...
function registerServiceWorker() {
  ...
  const registerCurrentServiceWorker = async () => {
    try {
      const registration = await navigator.serviceWorker.register("sw.js?v=23.3.55", { updateViaCache: "none" });
