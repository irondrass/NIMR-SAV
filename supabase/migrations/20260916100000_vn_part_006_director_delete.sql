-- Migration: vn_part_006_director_delete
-- Authoritative deletion of VN-PART removals by canonical Directeur role.
-- Preserves immutable audit trail by dropping restrictive foreign key and recording DELETE_REMOVAL audit event.
begin;
set local lock_timeout = '3s';
set local statement_timeout = '60s';

-- 1. Update public.vn_part_audit_events action check constraint to include DELETE_REMOVAL
alter table public.vn_part_audit_events
  drop constraint if exists vn_part_audit_events_action_check;

alter table public.vn_part_audit_events
  add constraint vn_part_audit_events_action_check check (
    action in (
      'CREATE_REQUEST',
      'APPROVE',
      'REFUSE',
...
-- 2. Drop restrictive foreign key constraint on public.vn_part_audit_events.
-- This ensures audit records can outlive their source removal while preserving
-- full immutable history and without violating relational constraints or weakening
-- the trg_vn_part_audit_immutable trigger.
alter table public.vn_part_audit_events
  drop constraint if exists vn_part_audit_events_removal_id_fkey;

-- 3. Replace nimr_internal.nimr_apply_vn_part_action_v1 with DELETE_REMOVAL support
...
-- 4. Re-declare public wrapper function for schema consistency
