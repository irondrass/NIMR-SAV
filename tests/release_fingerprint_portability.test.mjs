...
// -------------------------------------------------------------
test("K. XLSX vendor asset sensitivity & sealed v23.3.51 match: mutating 1 byte in vendor/xlsx.mini.min.js changes fingerprint; canonical matches sealed", () => {
  const normalHash = computeReleaseFingerprint(repositoryRoot, RELEASE_OWNED_RUNTIME_FILES);
  assert.equal(normalHash, SEALED_RELEASE_FINGERPRINTS["v23.3.51"], "Canonical release fingerprint must match sealed v23.3.51");
