...
console.log('Demarrage tests v23.2.3 roles and governance hardening...');
...
assert.match(versionSource, /NIMR_BUILD\s*=\s*"v23\.2\.3"/, 'version.js v23.2.3 attendu');
[...indexSource.matchAll(/\?v=(\d+\.\d+(?:\.\d+)?)/g)].forEach((match) => {
  assert.equal(match[1], '23.2.3', `reference index.html incoherente: ?v=${match[1]}`);
});
...
console.log('Tests v23.2.3 roles and governance hardening OK');
