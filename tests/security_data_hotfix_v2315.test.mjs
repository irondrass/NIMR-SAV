...
const EXPECTED_APP_VERSION = currentBuild.appVersion;
const EXPECTED_QUERY_VERSION = currentBuild.queryVersion;
const EXPECTED_CACHE_NAME = currentBuild.cacheName;

console.log(`Démarrage tests sécurité données + version/cache ${EXPECTED_APP_VERSION}...`);
...
assert.ok(swSource.includes(`const CACHE_NAME = "${EXPECTED_CACHE_NAME}"`), `sw.js doit exposer le cache PWA ${EXPECTED_CACHE_NAME}`);
assert.ok(stateSource.includes(`const APP_VERSION = "${EXPECTED_APP_VERSION}"`), `APP_VERSION doit être ${EXPECTED_APP_VERSION} dans state.js`);
assert.ok(versionSource.includes(`window.APP_VERSION = "${EXPECTED_APP_VERSION}"`), "version.js doit exposer APP_VERSION");
assert.ok(versionSource.includes(`window.NIMR_BUILD = "${currentBuild.buildVersion}"`), "version.js doit exposer NIMR_BUILD");
assert.equal(currentBuild.buildVersion, EXPECTED_APP_VERSION, "NIMR_BUILD doit être identique à APP_VERSION");
assert.ok(versionSource.includes(`window.NIMR_CACHE_NAME = "${EXPECTED_CACHE_NAME}"`), "version.js doit annoncer le cache courant");
assert.ok(appSource.includes(`serviceWorker.register("sw.js?v=${EXPECTED_QUERY_VERSION}"`), `app.js doit enregistrer sw.js?v=${EXPECTED_QUERY_VERSION}`);
getVersionedAssetQueries(indexSource).forEach((queryVersion) => {
  assert.equal(queryVersion, EXPECTED_QUERY_VERSION, `référence index.html incohérente: ?v=${queryVersion}`);
});
[
...
console.log(`Tests sécurité données + version/cache ${EXPECTED_APP_VERSION} OK`);
