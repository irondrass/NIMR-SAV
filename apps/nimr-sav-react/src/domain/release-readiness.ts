...
export function validateVersionReadiness(appVersion: string): ReadinessCheckResult {
  ...
  if (!appVersion) {
    ...
    if (appVersion !== 'v24.0.0-alpha.15') {
      blockers.push(`Version mismatch: expected 'v24.0.0-alpha.15', got '${appVersion}'.`);
    }
...
export function getReleaseReadinessChecklist(): { id: string; label: string; checked: boolean }[] {
  return [
    { id: 'version_rc1', label: 'Version calée sur v24.0.0-alpha.15', checked: true },
...
export function validateReleaseReadiness(
  ...
  const versionInput = options?.appVersion || 'v24.0.0-alpha.15';
