...
check("A UX release is consistently v23.3.9 across runtime cache surfaces", () => {
  assert.match(versionSource, /window\.APP_VERSION = "v23\.3\.9";/u);
  assert.match(versionSource, /window\.NIMR_BUILD = "v23\.3\.9";/u);
  assert.match(versionSource, /window\.NIMR_CACHE_NAME = "nimr-sav-v23\.3\.9";/u);
  assert.match(stateSource, /const APP_VERSION = "v23\.3\.9";/u);
  assert.match(appSource, /pdf\.worker\.min\.js\?v=23\.3\.9/u);
  assert.match(swSource, /const CACHE_NAME = "nimr-sav-v23\.3\.9";/u);
  assert.match(indexSource, /styles\.css\?v=23\.3\.9/u);
  assert.match(indexSource, /app\.js\?v=23\.3\.9/u);
  assert.match(offlineSource, /styles\.css\?v=23\.3\.9/u);
});
