...
test("B. NEW CACHE VERSION: bumping cache name purges old cache upon activation", async () => {
  ...
  const oldCache = await harness.context.caches.open("nimr-sav-v23.3.37");
  await oldCache.put(`${origin}/NIMR-SAV/js/storage.js?v=23.3.37`, new MockResponse("old-v37"));

  await harness.context.caches.open("nimr-sav-v23.3.38");
  await harness.triggerActivate();

  const keys = await harness.context.caches.keys();
  assert.ok(!keys.includes("nimr-sav-v23.3.37"), "Old cache nimr-sav-v23.3.37 must be deleted on activate");
  assert.ok(keys.includes("nimr-sav-v23.3.38"), "Current cache nimr-sav-v23.3.38 must be preserved");
});

test("C. HTML/JS VERSION MISMATCH: a foreign query version cannot reuse the active release cache entry", async () => {
  const harness = createSwHarness({
    [`${origin}/NIMR-SAV/js/unrelated.js?v=23.3.39`]: new MockResponse("network-v33"),
  });
  const cache = await harness.context.caches.open("nimr-sav-v23.3.38");
  await cache.put(`${origin}/NIMR-SAV/js/unrelated.js?v=23.3.38`, new MockResponse("cached-v32"));

  const res = await harness.dispatchFetch(`${origin}/NIMR-SAV/js/unrelated.js?v=23.3.39`);
...
test("G. OFFLINE CONTROL: when offline, cached assets resolve immediately", async () => {
  const storageUrl = `${origin}/NIMR-SAV/js/storage.js?v=23.3.38`;
  const harness = createSwHarness({});
  const cache = await harness.context.caches.open("nimr-sav-v23.3.38");
...
test("AA. VERSION BUMP COMPLETE: all 7 production files agree on v23.3.38 and contain zero runtime v23.3.37 literals", () => {
  ...
  assert.match(versionSrc, /window\.APP_VERSION = "v23\.3\.38";/);
  assert.match(versionSrc, /window\.NIMR_BUILD = "v23\.3\.38";/);
  assert.match(versionSrc, /window\.NIMR_CACHE_NAME = "nimr-sav-v23\.3\.38";/);
  assert.match(stateSrc, /const APP_VERSION = "v23\.3\.38";/);
  assert.match(swSrc, /const CACHE_NAME = "nimr-sav-v23\.3\.38";/);
  assert.match(appSrc, /vendor\/pdf\.worker\.min\.js\?v=23\.3\.38/);
  assert.match(appSrc, /sw\.js\?v=23\.3\.38/);
  assert.match(estimateSrc, /vendor\/pdf\.worker\.min\.js\?v=23\.3\.38/);
  assert.match(offlineSrc, /styles\.css\?v=23\.3\.38/);

  // Assert no runtime code in the 7 production files contains v23.3.37
  const codeFiles = [
  ...
  for (const { file, src } of codeFiles) {
    assert.doesNotMatch(src, /23\.3\.37/, `${file} must not contain any 23.3.37 literal`);
  }
});
...
test("AB. ACTIVE BUCKET ISOLATION: Worker B active with Cache B returns B even when Cache C exists with C", async () => {
  const sharedCaches = new Map();
  const network = new Map();
  const assetUrl = `${origin}/NIMR-SAV/js/storage.js?v=23.3.38`;

  const workerB = createSwHarness({}, { cacheStorage: sharedCaches, network });
  const cacheB = await workerB.context.caches.open("nimr-sav-v23.3.38");
  await cacheB.put(assetUrl, new MockResponse("// Storage Release B"));

  // Open and populate Cache C with C content
  const cacheC = await workerB.context.caches.open("nimr-sav-v23.3.39");
...
test("AC. WAITING CACHE INVISIBLE: matching URL in Cache C never satisfies Worker B lookup", async () => {
  const sharedCaches = new Map();
  const network = new Map();
  const missingAssetUrl = `${origin}/NIMR-SAV/js/state.js?v=23.3.38`;

  const workerB = createSwHarness({}, { cacheStorage: sharedCaches, network });
  // Cache B does NOT contain missingAssetUrl
  await workerB.context.caches.open("nimr-sav-v23.3.38");
...
test("AN. PDF WORKER CONTRACT: app.js, estimate-import.js, and sw.js precache PDF worker URLs agree exactly", () => {
  ...
  assert.equal(appMatch[1], "vendor/pdf.worker.min.js?v=23.3.38");
  assert.equal(estimateMatch[1], "vendor/pdf.worker.min.js?v=23.3.38");
  assert.ok(swSrc.includes('"./vendor/pdf.worker.min.js?v=23.3.38"'));
});
...
test("AO. OFFLINE: complete release usable entirely from active cache when offline", async () => {
  ...
  await cache.put(`${origin}/NIMR-SAV/js/storage.js?v=23.3.38`, new MockResponse("// Storage"));
  await cache.put(`${origin}/NIMR-SAV/styles.css?v=23.3.38`, new MockResponse("/* Styles */"));

  // Completely empty network
  network.clear();

  const navRes = await worker.dispatchFetch(`${origin}/NIMR-SAV/`, "navigate");
  const jsRes = await worker.dispatchFetch(`${origin}/NIMR-SAV/js/storage.js?v=23.3.38`);
  const cssRes = await worker.dispatchFetch(`${origin}/NIMR-SAV/styles.css?v=23.3.38`);
...
const EXPECTED_RELEASE_VERSION = "v23.3.38";
...
test("AT. SEALED RELEASE MUTATION: modifying runtime source under any sealed v23.3.21 through v23.3.38 fails validation", () => {
  const originalContent = fs.readFileSync(path.join(rootDir, "js/storage.js"));
  const mutatedContent = Buffer.concat([originalContent, Buffer.from(" ")]);
...
test("AV. NEW RELEASE ENTRY: bumping release to v23.3.39 with new sealed entry passes while v23.3.21 through v23.3.38 stay sealed", () => {
  const simulatedRegistry = {
    ...SEALED_RELEASE_FINGERPRINTS,
    "v23.3.39": "1111111111111111111111111111111111111111111111111111111111111111",
  };
...
test("AS. isReleaseAsset MEMBERSHIP: only declared ASSETS with ?v=23.3.38 are classified as release assets", async () => {
  ...
  const arbitraryUrls = [
    `${origin}/NIMR-SAV/api/data?v=23.3.38`,
    `${origin}/NIMR-SAV/some-other-page?v=23.3.38&extra=true`,
    `${origin}/other-app/js/storage.js?v=23.3.38`,
  ];
