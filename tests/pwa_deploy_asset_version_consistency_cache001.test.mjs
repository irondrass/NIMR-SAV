...
test("B. NEW CACHE VERSION: bumping cache name purges old cache upon activation", async () => {
  ...
  const oldCache = await harness.context.caches.open("nimr-sav-v23.3.21");
  await oldCache.put(`${origin}/NIMR-SAV/js/storage.js?v=23.3.21`, new MockResponse("old-v21"));

  await harness.context.caches.open("nimr-sav-v23.3.22");
  await harness.triggerActivate();

  const keys = await harness.context.caches.keys();
  assert.ok(!keys.includes("nimr-sav-v23.3.21"), "Old cache nimr-sav-v23.3.21 must be deleted on activate");
  assert.ok(keys.includes("nimr-sav-v23.3.22"), "Current cache nimr-sav-v23.3.22 must be preserved");
});

test("C. HTML/JS VERSION MISMATCH: if index.html requests ?v=23.3.23 while cache only has ?v=23.3.22, it falls back to networkFirst", async () => {
  const harness = createSwHarness({
    [`${origin}/NIMR-SAV/js/unrelated.js?v=23.3.23`]: new MockResponse("network-v23"),
  });
  const cache = await harness.context.caches.open("nimr-sav-v23.3.22");
  await cache.put(`${origin}/NIMR-SAV/js/unrelated.js?v=23.3.22`, new MockResponse("cached-v22"));

  const res = await harness.dispatchFetch(`${origin}/NIMR-SAV/js/unrelated.js?v=23.3.23`);
  assert.equal(await res.text(), "network-v23");
});
...
test("G. OFFLINE CONTROL: when offline, cached assets resolve immediately", async () => {
  const storageUrl = `${origin}/NIMR-SAV/js/storage.js?v=23.3.22`;
  const harness = createSwHarness({});
  const cache = await harness.context.caches.open("nimr-sav-v23.3.22");
...
test("AA. VERSION BUMP COMPLETE: all 7 production files agree on v23.3.22 and contain zero runtime v23.3.21 literals", () => {
  ...
  assert.match(versionSrc, /window\.APP_VERSION = "v23\.3\.22";/);
  assert.match(versionSrc, /window\.NIMR_BUILD = "v23\.3\.22";/);
  assert.match(versionSrc, /window\.NIMR_CACHE_NAME = "nimr-sav-v23\.3\.22";/);
  assert.match(stateSrc, /const APP_VERSION = "v23\.3\.22";/);
  assert.match(swSrc, /const CACHE_NAME = "nimr-sav-v23\.3\.22";/);
  assert.match(appSrc, /vendor\/pdf\.worker\.min\.js\?v=23\.3\.22/);
  assert.match(appSrc, /sw\.js\?v=23\.3\.22/);
  assert.match(estimateSrc, /vendor\/pdf\.worker\.min\.js\?v=23\.3\.22/);
  assert.match(offlineSrc, /styles\.css\?v=23\.3\.22/);

  // Assert no runtime code in the 7 production files contains v23.3.21 (ignoring historical comments in sw.js)
  const codeFiles = [
  ...
  for (const { file, src } of codeFiles) {
    assert.doesNotMatch(src, /23\.3\.21/, `${file} must not contain any 23.3.21 literal`);
  }
});
...
test("AB. ACTIVE BUCKET ISOLATION: Worker B active with Cache B returns B even when Cache C exists with C", async () => {
  const sharedCaches = new Map();
  const network = new Map();
  const assetUrl = `${origin}/NIMR-SAV/js/storage.js?v=23.3.22`;

  const workerB = createSwHarness({}, { cacheStorage: sharedCaches, network });
  const cacheB = await workerB.context.caches.open("nimr-sav-v23.3.22");
  await cacheB.put(assetUrl, new MockResponse("// Storage Release B"));

  // Open and populate Cache C with C content
  const cacheC = await workerB.context.caches.open("nimr-sav-v23.3.23");
...
test("AC. WAITING CACHE INVISIBLE: matching URL in Cache C never satisfies Worker B lookup", async () => {
  const sharedCaches = new Map();
  const network = new Map();
  const missingAssetUrl = `${origin}/NIMR-SAV/js/state.js?v=23.3.22`;

  const workerB = createSwHarness({}, { cacheStorage: sharedCaches, network });
  // Cache B does NOT contain missingAssetUrl
  await workerB.context.caches.open("nimr-sav-v23.3.22");
...
test("AD. NO RELEASE BACKGROUND REVALIDATION: cache hit for versioned JS/CSS makes zero network calls and zero cache puts", async () => {
  const sharedCaches = new Map();
  const network = new Map();
  const jsUrl = `${origin}/NIMR-SAV/js/storage.js?v=23.3.22`;
  const cssUrl = `${origin}/NIMR-SAV/styles.css?v=23.3.22`;
  ...
  const cache = await worker.context.caches.open("nimr-sav-v23.3.22");
...
test("AG. CHECK_UPDATE NO-OP: CHECK_UPDATE message does not call precache, mutate Cache B, or emit false CACHE_REFRESHED", async () => {
  ...
  const worker = createSwHarness({}, { cacheStorage: sharedCaches, network });
  const cache = await worker.context.caches.open("nimr-sav-v23.3.22");
  await cache.put(`${origin}/NIMR-SAV/js/storage.js?v=23.3.22`, new MockResponse("// initial B bytes"));

  const snapshotBefore = Array.from(sharedCaches.get("nimr-sav-v23.3.22")?.entries() || [])
    .map(([k, v]) => [k, v.body]);
  ...
  // Cache B unchanged
  const stored = await cache.match(`${origin}/NIMR-SAV/js/storage.js?v=23.3.22`);
...
test("AI. CONTROLLED WAITING: new worker installs into its own bucket, stays waiting, old worker continues serving", async () => {
  ...
  // Worker C installs
  const swSourceC = swSource.replace(/23\.3\.22/g, "23.3.23");
...
test("AL. POST-ACTIVATION: Worker C active serves complete Release C with no Cache B dependency", async () => {
  ...
  const swSourceC = swSource.replace(/23\.3\.22/g, "23.3.23");
  const workerC = createSwHarness({}, { cacheStorage: sharedCaches, network, swSource: swSourceC });
  const cacheC = await workerC.context.caches.open("nimr-sav-v23.3.23");
  await cacheC.put(`${origin}/NIMR-SAV/`, new MockResponse("<!DOCTYPE html><html>App C</html>"));
  await cacheC.put(`${origin}/NIMR-SAV/js/storage.js?v=23.3.23`, new MockResponse("// Storage C"));

  await workerC.triggerActivate();

  const nav = await workerC.dispatchFetch(`${origin}/NIMR-SAV/`, "navigate");
  const js = await workerC.dispatchFetch(`${origin}/NIMR-SAV/js/storage.js?v=23.3.23`);
...
test("AN. PDF WORKER CONTRACT: app.js, estimate-import.js, and sw.js precache PDF worker URLs agree exactly", () => {
  ...
  assert.equal(appMatch[1], "vendor/pdf.worker.min.js?v=23.3.22");
  assert.equal(estimateMatch[1], "vendor/pdf.worker.min.js?v=23.3.22");
  assert.ok(swSrc.includes('"./vendor/pdf.worker.min.js?v=23.3.22"'));
});
...
test("AO. OFFLINE: complete release usable entirely from active cache when offline", async () => {
  ...
  await cache.put(`${origin}/NIMR-SAV/js/storage.js?v=23.3.22`, new MockResponse("// Storage"));
  await cache.put(`${origin}/NIMR-SAV/styles.css?v=23.3.22`, new MockResponse("/* Styles */"));

  // Completely empty network
  network.clear();

  const navRes = await worker.dispatchFetch(`${origin}/NIMR-SAV/`, "navigate");
  const jsRes = await worker.dispatchFetch(`${origin}/NIMR-SAV/js/storage.js?v=23.3.22`);
  const cssRes = await worker.dispatchFetch(`${origin}/NIMR-SAV/styles.css?v=23.3.22`);
...
const EXPECTED_RELEASE_VERSION = "v23.3.22";
...
test("AT. SEALED RELEASE MUTATION: modifying runtime source under sealed v23.3.21 or v23.3.22 fails validation", () => {
  const originalContent = fs.readFileSync(path.join(rootDir, "js/storage.js"));
  const mutatedContent = Buffer.concat([originalContent, Buffer.from(" ")]);
...
test("AV. NEW RELEASE ENTRY: bumping release to v23.3.23 with new sealed entry passes while v23.3.21 and v23.3.22 stay sealed", () => {
  const simulatedRegistry = {
    ...SEALED_RELEASE_FINGERPRINTS,
    "v23.3.23": "1111111111111111111111111111111111111111111111111111111111111111",
  };
...
test("AS. isReleaseAsset MEMBERSHIP: only declared ASSETS with ?v=23.3.22 are classified as release assets", async () => {
  ...
  const arbitraryUrls = [
    `${origin}/NIMR-SAV/api/data?v=23.3.22`,
    `${origin}/NIMR-SAV/some-other-page?v=23.3.22&extra=true`,
    `${origin}/other-app/js/storage.js?v=23.3.22`,
  ];
  for (const url of arbitraryUrls) {
    ...
    // Therefore even if an arbitrary URL with ?v=23.3.22 reaches isReleaseAsset and it returns true,
