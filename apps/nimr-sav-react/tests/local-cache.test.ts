...
describe('Local Cache Domain Snapshotting & Restore', () => {
  ...
  it('builds a correct LocalSnapshot structure', () => {
    const appVersion = '24.0.0-alpha.20';
  ...
  it('invalidates a snapshot with incompatible schema version', () => {
    const snap = {
      appVersion: '24.0.0-alpha.20',
      schemaVersion: 23,
      cases: mockCases,
      logs: mockLogs,
    };
    const val = validateLocalSnapshot(snap, '24.0.0-alpha.20');
  ...
  it('invalidates a snapshot with different appVersion', () => {
    ...
    const val = validateLocalSnapshot(snap, '24.0.0-alpha.20-different');
    expect(val.valid).toBe(false);
