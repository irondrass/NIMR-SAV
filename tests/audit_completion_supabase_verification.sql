...
begin
  ...
  if exists(select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef) then raise exception 'FAIL: privileged implementation remains exposed'; end if;
  ...
  perform set_config('role','postgres',true);
  insert into public.sync_entities(workshop_id,entity_type,entity_id,payload,entity_version,last_operation_id)
    values(w,'booking',c||':booking',jsonb_build_object('caseId',c,'type','work','status','planned'),0,c||':booking');
  perform set_config('role','authenticated',true);
  begin
    perform public.nimr_apply_quality_review_v2(w,c,'validated','Verification',c||':quality',0);
    raise exception 'FAIL: outstanding operation accepted';
  exception when check_violation then null; end;
  perform set_config('role','postgres',true);
  update public.sync_entities set payload=jsonb_set(payload,'{status}','"completed"') where workshop_id=w and entity_id=c||':booking';
  perform set_config('role','authenticated',true);
  ...
  exception when insufficient_privilege then null; end;
  perform set_config('role','postgres',true);
  -- Nonmember JWT must fail at the trigger boundary, even for a SQL owner.
  begin
    update public.sync_entities set payload=payload||'{"clientCommitment":{"promisedAt":"2026-09-08T16:00:00Z"}}' where workshop_id=w and entity_id=c;
    raise exception 'FAIL: unauthorized promise accepted';
  exception when insufficient_privilege then null; end;
  -- Empty defaults must not block historical technician payloads after upgrade.
  update public.sync_entities set payload=payload||'{"clientCommitment":{"promisedAt":"","nextContactAt":"","lastContactAt":"","note":""},"exceptionFollowup":{"ownerId":"","dueAt":"","note":""}}' where workshop_id=w and entity_id=c;
  perform set_config('request.jwt.claim.sub',actor::text,true);
  update public.sync_entities set payload=payload||'{"clientCommitment":{"promisedAt":"2026-09-08T16:00:00Z","note":"Synthetic promise"}}' where workshop_id=w and entity_id=c;
end;
$test$;
rollback;
select 'PASS: private API, outstanding operation, authorized promise and default normalization, role denial, private receipts, physical handover gate, unfinished work, stale decision, valid review, preserved fields and idempotent retry; synthetic data rolled back' as verification;
