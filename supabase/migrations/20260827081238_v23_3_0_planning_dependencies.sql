...
begin
  ...
  begin
    ...
    loop
      ...
      select coalesce(array_agg(distinct dep order by dep), '{}'::text[])
      into dependencies_value
      from (
        select nullif(trim(dependency_token.value), '') as dep
      ...
      loop
        select max(candidate.end_at)
        into dependency_end_value
        from public.planning_slots candidate
        where candidate.workshop_id = p_workshop_id
          and candidate.repair_order_id = repair_order_id_value
          and candidate.deleted_at is null
          and (
            candidate.id::text = dependency_ref
            or candidate.local_id = dependency_ref
            or candidate.task_id = dependency_ref
            or candidate.step_key = dependency_ref
          );
        ...
        if dependency_end_value is null then
          conflict_result := jsonb_build_object(
            ...
            'message', 'Une dépendance planning est introuvable.'
          );
          raise exception 'NIMR_RESERVATION_CONFLICT:%', conflict_result::text
            using errcode = 'P0001';
        end if;

        if dependency_end_value > start_value then
          conflict_result := jsonb_build_object(
            'ok', false,
            'status', 'conflict',
            ...
            'slotId', slot_id_value,
            'dependency', dependency_ref,
            'dependencyEndAt', dependency_end_value,
            'startAt', start_value,
            'message', 'Le créneau démarre avant la fin de sa dépendance.'
          );
          raise exception 'NIMR_RESERVATION_CONFLICT:%', conflict_result::text
            using errcode = 'P0001';
        end if;
      ...
      if found then
        ...
        insert into public.planning_slots (
          ...
          primary_resource_id, equipment_resource_ids, task_id, step_key, dependencies, title,
      ...
      loop
        select greatest(1, coalesce(resource.simultaneous_capacity, resource.capacity, 1))
        ...
        resource_units_value := greatest(1, coalesce(
...
values ('23.3.0-planning-dependencies', 'Conservation et validation atomique des dépendances planning.')
