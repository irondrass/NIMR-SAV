...
test("23 hard baselines unchanged", () => { for (const rel of ["js/planning.js","js/state.js","js/business-rules-v2187.js","js/version.js","sw.js"]) assert.equal(unchanged(rel),true,rel); });
test("24 release identity stays v23.3.26", () => { assert.match(version,/^window\.APP_VERSION = "v23\.3\.26";$/mu); assert.match(version,/^window\.NIMR_BUILD = "v23\.3\.26";$/mu); assert.match(version,/^window\.NIMR_CACHE_NAME = "nimr-sav-v23\.3\.26";$/mu); });
